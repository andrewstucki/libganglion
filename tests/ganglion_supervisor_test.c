...
void test_ganglion_supervisor_new(void **state) {
  ...
  assert_int_equal(supervisor->consumer_size, 0);
  assert_null(supervisor->consumers);
  assert_int_equal(supervisor->status, GANGLION_THREAD_INITIALIZED);
...
void test_ganglion_supervisor_cleanup_running(void **state) {
  struct ganglion_supervisor * supervisor = ganglion_supervisor_new();
  supervisor->status = GANGLION_THREAD_STARTED;

  expect_assert_failure(ganglion_supervisor_cleanup(supervisor));

  supervisor->status = GANGLION_THREAD_INITIALIZED;
  ganglion_supervisor_cleanup(supervisor);
  ganglion_shutdown();
}
...
void test_ganglion_supervisor_register(void **state) {
  ...
  assert_int_equal(supervisor->consumer_size, 1);
  assert_non_null(supervisor->consumers);
  assert_ptr_equal(supervisor->consumers[0], consumer_one);

  consumer_size = ganglion_supervisor_register(supervisor, consumer_two);
  assert_int_equal(consumer_size, 2);
  assert_int_equal(supervisor->consumer_size, 2);
  assert_non_null(supervisor->consumers);
  assert_ptr_equal(supervisor->consumers[1], consumer_two);
